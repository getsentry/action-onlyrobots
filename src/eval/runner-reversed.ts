import { LLMEvaluator } from '../llm-evaluator-reversed';
...
export class EvalRunnerReversed {
  ...
  async runEvaluation(options: { limit?: number; concurrency?: number } = {}) {
    ...
    console.log('🚀 Starting PR evaluation with REVERSED heuristic (default to AI)...\n');
  ...
  private displaySummary(results: any[], totalDuration: number) {
    ...
    const correctAvgConfidence =
      results
        ...
        results.filter((r) => r.result && r.isCorrect).length || 0;
    ...
    console.log('📊 EVALUATION SUMMARY (Reversed Heuristic)');
    console.log('='.repeat(60));
    ...
    if (results.filter((r) => !r.isCorrect).length > 0) {
      ...
      results
        ...
        .forEach((r) => {
          ...
          if (r.result?.indicators?.length > 0) {
            console.log(`      Indicators: ${r.result.indicators.join(', ')}`);
          }
        });
    }
  ...
  private async saveResults(results: any[]) {
    ...
    const filename = `eval-reversed-${timestamp}.json`;
...
if (require.main === module) {
  const runner = new EvalRunnerReversed();
