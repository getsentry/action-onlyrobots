...
function formatResults(result, format, verbose, prRef, showDiffs = false) {
    if (format === 'json') {
...
async function main() {
    ...
    try {
        ...
        const evaluator = new llm_evaluator_1.LLMEvaluator({
        ...
        formatResults(result, options.format, !!options.verbose, prRef, !!options['show-diffs']);
    }
    catch (error) {
        console.error(`❌ Error: ${error instanceof Error ? error.message : String(error)}`);
